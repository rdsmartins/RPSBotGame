...
public class BotVsHumanGame {
	...
	private int roundsToPlayAtThisMatch;
	...
	public void startGame() {
		...
		startMatch(game); // at least one match always
	...
	public void startMatch(Game game) {
		...
		this.roundsToPlayAtThisMatch = terminal.getNumberOfRounds();

		roundsPlayed = 1;
		match = new Match(game.getPlayer1(), game.getPlayer2(), this.roundsToPlayAtThisMatch);

		while (this.hasMoreRoundsToPlayInThisMatch() && !match.hasRoundBestOfWinner()) {
			...
			Result result = round.play(this.player1, this.player2); // A RESULT CAN BE A DRAW

			if (round.hasAWinner()) {
				match.addMatchResult(result);
				terminal.printMatchResultMessage(round.getPlayer1Gesture(), round.getPlayer2Gesture(),
						round.getWinnerName());
				roundsPlayed++;
				this.nextMatch();
	...
	private boolean hasMoreRoundsToPlayInThisMatch() {
		if (this.roundsToPlayAtThisMatch > 0) {
			return true;
		}
		return false;
	}
